...
- **Update-safe.** The mod finds the game code it needs when the game starts. If a game update changes that code, the mod switches itself off and writes the reason to `OreSweep.log`. It doesn't crash the game.
...
3. Start the game. `OreSweep.log` appears next to the dll and should end with `OK: hook installed`.

**Uninstalling:** delete `dsound.dll` (and `OreSweep.ini` / `OreSweep.log`).
...
| `DebugLog` | `0` | `1` writes the kept / skipped counts for every ore-only drag to `OreSweep.log`. |
...
| Path | What |
|---|---|
| `src/main.c` | The mod: DLL entry, config and log, signature scan, the cell filter. |
...
Built for game version 0.7.207. Later versions work as long as the patched code hasn't changed. Check the log after an update.
